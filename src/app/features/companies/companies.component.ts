...
export class CompaniesComponent implements OnInit {
  ...
  add()  { this.dialog.open(CompanyDialogComponent,{width:'480px',data:{mode:'create'}})
           .afterClosed().subscribe(ok => { if(ok){ this.sb.open('Company created','OK',{duration:1500}); this.load(); } }); }
  edit(r: Company) { this.dialog.open(CompanyDialogComponent,{width:'480px',data:{mode:'edit', company:r}})
           .afterClosed().subscribe(ok => { if(ok){ this.sb.open('Company updated','OK',{duration:1500}); this.load(); } }); }
